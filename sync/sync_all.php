...
{
    $db->exec("CREATE TABLE IF NOT EXISTS sync_order_summary (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS sync_orders (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS sync_order_items (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    // sync_products dengan UNIQUE KEY di product_code (bukan external_product_id)
    $db->exec("CREATE TABLE IF NOT EXISTS sync_products (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    $db->exec("DELETE t1 FROM sync_products t1 INNER JOIN sync_products t2 WHERE t1.id < t2.id AND t1.product_code = t2.product_code AND t1.product_code IS NOT NULL");
    $oldIdx = $db->query("SHOW INDEX FROM sync_products WHERE Key_name = 'uk_external_product_id'")->fetchAll();
    if (count($oldIdx) > 0) {
        $db->exec("ALTER TABLE sync_products DROP INDEX uk_external_product_id");
    }
    $newIdx = $db->query("SHOW INDEX FROM sync_products WHERE Key_name = 'uk_product_code'")->fetchAll();
    if (count($newIdx) === 0) {
...
function cleanOldLogs(): void
{
    ...
    if (!is_dir($logDir))
        return;
    $cutoff = time() - (SYNC_LOG_RETENTION_DAYS * 86400);
...
try {
    ...
    $logStmt = $db->prepare("INSERT INTO sync_log (sync_type, status, started_at) VALUES ('sync_all', 'running', NOW())");
    $logStmt->execute();
    ...
    $updateStmt = $db->prepare("UPDATE sync_log SET status=:status, finished_at=NOW(), records_inserted=:inserted, records_updated=:updated, records_failed=:failed, total_records=:total, pages_fetched=:pages, error_message=:error WHERE id=:id");
    $updateStmt->execute([
        ':status' => $status,
    ...
    if (isset($db) && isset($syncLogId)) {
        try {
            $db->prepare("UPDATE sync_log SET status='failed', finished_at=NOW(), error_message=:error WHERE id=:id")->execute([':error' => $e->getMessage(), ':id' => $syncLogId]);
